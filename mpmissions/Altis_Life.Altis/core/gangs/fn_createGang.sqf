...
if (_length > 32) exitWith {[ localize "STR_GNOTF_Over32",false,"fast"] call AYU_Client_fnc_notification_system;};
_badChar = false;
{if (!(_x in _allowed)) exitWith {_badChar = true;};} forEach _chrByte;
if (_badChar) exitWith {[ localize "STR_GNOTF_IncorrectChar",false,"fast"] call AYU_Client_fnc_notification_system;};
if (BANK < (LIFE_SETTINGS(getNumber,"gang_price"))) exitWith {[ format [localize "STR_GNOTF_NotEnoughMoney",[((LIFE_SETTINGS(getNumber,"gang_price")) - BANK)] call life_fnc_numberText],false,"fast"] call AYU_Client_fnc_notification_system;};
...
[ localize "STR_NOTF_SendingData",false,"fast"] call AYU_Client_fnc_notification_system;
closeDialog 0;
