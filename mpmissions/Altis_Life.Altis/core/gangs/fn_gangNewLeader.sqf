...
if ((lbCurSel 2621) isEqualTo -1) exitWith {[ localize "STR_GNOTF_TransferSelect",false,"fast"] call AYU_Client_fnc_notification_system;};
_unit = call compile format ["%1",CONTROL_DATA(2621)];

if (isNull _unit) exitWith {}; //Bad unit?
if (_unit == player) exitWith {[ localize "STR_GNOTF_TransferSelf",false,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_action) then {
    ...
    if (_unitID isEqualTo "") exitWith {[ localize "STR_GNOTF_badUID",false,"fast"] call AYU_Client_fnc_notification_system;}; //Unlikely?
    ...
    [ localize "STR_GNOTF_TransferCancel",false,"fast"] call AYU_Client_fnc_notification_system;
};
