...
if (_action) then {
    if (BANK < _upgradePrice) exitWith {
        ...
        ],false,"fast"] call AYU_Client_fnc_notification_system;
    };
    BANK = BANK - _upgradePrice;
    [1] call SOCK_fnc_updatePartial;
    group player setVariable ["gang_maxMembers",_slotUpgrade,true];
    [ parseText format [localize "STR_GNOTF_UpgradeSuccess",_maxMembers,_slotUpgrade,[_upgradePrice] call life_fnc_numberText],false,"fast"] call AYU_Client_fnc_notification_system;
    ...
    [ localize "STR_GNOTF_UpgradeCancel",false,"fast"] call AYU_Client_fnc_notification_system;
};
