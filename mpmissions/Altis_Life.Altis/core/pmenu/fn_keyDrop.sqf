...
if (_sel isEqualTo -1) exitWith {
    [ localize "STR_NOTF_noDataSelected",true,"fast"] call AYU_Client_fnc_notification_system;
};

if (_list lbData _sel isEqualTo "") exitWith {
    [ localize "STR_NOTF_didNotSelectVehicle",true,"fast"] call AYU_Client_fnc_notification_system;
};
...
if (_vehicle isKindOf "House_F") exitWith {
    [ localize "STR_NOTF_cannotRemoveHouseKeys",true,"fast"] call AYU_Client_fnc_notification_system;
};

// Solve stupidness
if (objectParent player isEqualTo _vehicle && {locked _vehicle isEqualTo 2}) exitWith {
    [ localize "STR_NOTF_cannotDropKeys",true,"fast"] call AYU_Client_fnc_notification_system;
};
