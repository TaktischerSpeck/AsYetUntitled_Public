...
if (!(_building isKindOf "House_F")) exitWith {[ localize "STR_ISTR_Bolt_NotNear",false,"fast"] call AYU_Client_fnc_notification_system;};
if (((nearestObject [_pos,"Land_Dome_Big_F"]) == _building || (nearestObject [_pos,_vaultHouse]) == _building) && (west countSide playableUnits < (LIFE_SETTINGS(getNumber,"minimum_cops")))) exitWith {
    [ format [localize "STR_Civ_NotEnoughCops",(LIFE_SETTINGS(getNumber,"minimum_cops"))],false,"fast"] call AYU_Client_fnc_notification_system;
};
if ((typeOf _building) == _vaultHouse && (nearestObject [_pos,"Land_Dome_Big_F"]) getVariable ["locked",true]) exitWith {[ localize "STR_ISTR_Bolt_Exploit",false,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_door isEqualTo 0) exitWith {[ localize "STR_Cop_NotaDoor",false,"fast"] call AYU_Client_fnc_notification_system;}; //Not near a door to be broken into.
if ((_building getVariable [format ["bis_disabled_Door_%1",_door],0]) isEqualTo 0) exitWith {[ localize "STR_House_Raid_DoorUnlocked",false,"fast"] call AYU_Client_fnc_notification_system;};
