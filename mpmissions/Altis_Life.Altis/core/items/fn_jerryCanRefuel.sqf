...
if (count(nearestObjects [player,["Land_FuelStation_Feed_F","Land_fs_feed_F"],3.5]) isEqualTo 0) exitWith { [ localize "STR_ISTR_Jerry_Distance",true,"fast"] call AYU_Client_fnc_notification_system;};
if (life_action_inUse) exitWith {};
if !(isNull objectParent player) exitWith {};
if (player getVariable "restrained") exitWith {[ localize "STR_NOTF_isrestrained",true,"fast"] call AYU_Client_fnc_notification_system;};
if (player getVariable "playerSurrender") exitWith {[ localize "STR_NOTF_surrender",true,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_action) then {
    if (CASH < _fuelCost) exitWith {[ localize "STR_NOTF_NotEnoughMoney",true,"fast"] call AYU_Client_fnc_notification_system; life_action_inUse = false;};
    ...
    [ localize "STR_ISTR_Jerry_Refueled",true,"fast"] call AYU_Client_fnc_notification_system;
} else {
    [ localize "STR_NOTF_ActionCancel",true,"fast"] call AYU_Client_fnc_notification_system;
    closeDialog 0;
    life_action_inUse = false;
};