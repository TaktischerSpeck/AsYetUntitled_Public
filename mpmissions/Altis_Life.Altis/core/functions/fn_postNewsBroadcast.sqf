...
if (_length > _allowedLength) exitWith {[ format [localize "STR_News_HeaderLength",_allowedLength],true,"fast"] call AYU_Client_fnc_notification_system;};

{
    if (!(_x in _allowed)) exitWith {_badCharacter = true;};
} forEach _characterByte;

if (_badCharacter) exitWith {[ localize "STR_News_UnsupportedCharacter",true,"fast"] call AYU_Client_fnc_notification_system;};
