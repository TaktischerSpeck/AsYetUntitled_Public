...
if (_vehicle getVariable "NPC") exitWith {[ localize "STR_NPC_Protected",false,"fast"] call AYU_Client_fnc_notification_system;};
...
if (player distance _vehicle > 10) exitWith {[ localize "STR_NOTF_ImpoundingCancelled",false,"fast"] call AYU_Client_fnc_notification_system; life_action_inUse = false;};
...
if (count crew _vehicle isEqualTo 0) then {
    ...
    if (playerSide isEqualTo west) then {
            ...
            if (_vehicle in life_vehicles) then {
                [ format [localize "STR_NOTF_OwnImpounded",[_value] call life_fnc_numberText,_type],false,"fast"] call AYU_Client_fnc_notification_system;
                BANK = BANK - _value;
            } else {
                [ format [localize "STR_NOTF_Impounded",_type,[_value] call life_fnc_numberText],false,"fast"] call AYU_Client_fnc_notification_system;
                BANK = BANK + _value;
            };
    ...
    [ localize "STR_NOTF_ImpoundingCancelled",false,"fast"] call AYU_Client_fnc_notification_system;
};
