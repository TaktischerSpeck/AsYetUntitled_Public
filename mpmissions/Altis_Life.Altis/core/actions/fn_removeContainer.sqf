...
if (!(_house in life_vehicles)) exitWith {[ localize "STR_ISTR_Box_NotinHouse",false,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_action) then {
    ...
    if (_diff isEqualTo 0) exitWith {[ localize "STR_NOTF_InvFull",false,"fast"] call AYU_Client_fnc_notification_system;};
