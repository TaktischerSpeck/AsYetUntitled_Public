...
if (isNil "_classname" || _classname isEqualTo "") exitWith {
    [ localize "STR_Select_Vehicle_Pump",true,"fast"] call AYU_Client_fnc_notification_system;
    vehiclefuelList = [];
...
if (_car distance player > 10 && !(isNull objectParent player)) exitWith {
    [ localize "STR_Distance_Vehicle_Pump",true,"fast"] call AYU_Client_fnc_notification_system;
    vehiclefuelList = [];
    life_action_inUse = false;
    closeDialog 0;
};
...
if ((BANK - (_fueltoput * life_fuelPrices))> 0)then {
    ...
    [ localize "STR_NOTF_NotEnoughMoney",true,"fast"] call AYU_Client_fnc_notification_system;
};
