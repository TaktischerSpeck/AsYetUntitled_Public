...
if (isNull life_trunk_vehicle || !alive life_trunk_vehicle) exitWith {[ localize "STR_MISC_VehDoesntExist",true,"fast"] call AYU_Client_fnc_notification_system;};
...
if ((lbCurSel 3502) isEqualTo -1) exitWith {[ localize "STR_Global_NoSelection",true,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_num > _value) exitWith {[ localize "STR_MISC_NotEnough",true,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_ctrl == "money") then {
    ...
    if ([true,_ctrl,_num] call life_fnc_handleInv) then {
        ...
        [ localize "STR_NOTF_InvFull",true,"fast"] call AYU_Client_fnc_notification_system;
    };
};
