...
if (!isNil {_vehicle getVariable "mining"}) exitWith {
    [ localize "STR_NOTF_DeviceIsMining",true,"fast"] call AYU_Client_fnc_notification_system;
};
...
if ((_weight select 1) >= (_weight select 0)) exitWith {
    [ localize "STR_NOTF_DeviceFull",true,"fast"] call AYU_Client_fnc_notification_system;
...
if (_zone isEqualTo "") exitWith {
    [ localize "STR_NOTF_notNearResource",true,"fast"] call AYU_Client_fnc_notification_system;
    life_action_inUse = false;
};
