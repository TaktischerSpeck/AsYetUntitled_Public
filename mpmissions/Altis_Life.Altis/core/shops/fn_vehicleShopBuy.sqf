...
if ((lbCurSel 2302) isEqualTo -1) exitWith {[ localize "STR_Shop_Veh_DidntPick",true,"fast"] call AYU_Client_fnc_notification_system; closeDialog 0;};
...
if !([_conditions] call life_fnc_levelCheck) exitWith {[ localize "STR_Shop_Veh_NoLicense",true,"fast"] call AYU_Client_fnc_notification_system;};

private _colorIndex = lbValue[2304,(lbCurSel 2304)];

if (_purchasePrice < 0) exitWith {closeDialog 0;}; //Bad price entry
if (CASH < _purchasePrice) exitWith {[ format [localize "STR_Shop_Veh_NotEnough",[_purchasePrice - CASH] call life_fnc_numberText],true,"fast"] call AYU_Client_fnc_notification_system;closeDialog 0;};
...
if (_spawnPoint isEqualTo "") exitWith {[ localize "STR_Shop_Veh_Block",true,"fast"] call AYU_Client_fnc_notification_system; closeDialog 0;};
CASH = CASH - _purchasePrice;
[0] call SOCK_fnc_updatePartial;
[ format [localize "STR_Shop_Veh_Bought",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_purchasePrice] call life_fnc_numberText],true,"fast"] call AYU_Client_fnc_notification_system;
