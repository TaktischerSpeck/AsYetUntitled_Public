...
if (life_action_inUse) exitWith {[ localize "STR_NOTF_ActionInProc",false,"fast"] call AYU_Client_fnc_notification_system;};
if !(playerSide isEqualTo civilian) exitWith {[ localize "STR_NOTF_notAllowed",false,"fast"] call AYU_Client_fnc_notification_system;};
...
if (count _nearUnits > 1) exitWith {[ localize "STR_NOTF_PlayerNear",false,"fast"] call AYU_Client_fnc_notification_system;};

life_chopShop = _this select 3;
//Error check
if (count _nearVehicles isEqualTo 0) exitWith {titleText[localize "STR_Shop_NoVehNear","PLAIN"];};
if (!(createDialog "Chop_Shop")) exitWith {[ localize "STR_Shop_ChopShopError",false,"fast"] call AYU_Client_fnc_notification_system;};
