...
if !(_exit) exitWith {[ localize "STR_Shop_Veh_NoLicense",true,"fast"] call AYU_Client_fnc_notification_system;};
...
//Cop / Civ Pre Check
if (_shop in ["bruce","dive","reb","kart"] && {!(playerSide isEqualTo civilian)}) exitWith {[ localize "STR_Shop_NotaCiv",true,"fast"] call AYU_Client_fnc_notification_system; closeDialog 0;};
if (_shop == "reb" && {!license_civ_rebel}) exitWith {[ localize "STR_Shop_NotaReb",true,"fast"] call AYU_Client_fnc_notification_system; closeDialog 0;};
if (_shop == "cop" && {!(playerSide isEqualTo west)}) exitWith {[ localize "STR_Shop_NotaCop",true,"fast"] call AYU_Client_fnc_notification_system; closeDialog 0;};
if (_shop == "dive" && {!license_civ_dive}) exitWith {[ localize "STR_Shop_NotaDive",true,"fast"] call AYU_Client_fnc_notification_system; closeDialog 0;};
...
if (isClass(missionConfigFile >> "Licenses" >> life_clothing_store)) then {
    ...
    if !(LICENSE_VALUE(life_clothing_store,_flag)) exitWith {
        [ format [localize "STR_Shop_YouNeed",localize _displayName],true,"fast"] call AYU_Client_fnc_notification_system;
        closeDialog 0;
    };
};
