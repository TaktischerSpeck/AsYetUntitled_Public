...
if (_house getVariable ["house_owned",false]) exitWith {[ localize "STR_House_alreadyOwned",true,"fast"] call AYU_Client_fnc_notification_system;};
if (!isNil {(_house getVariable "house_sold")}) exitWith {[ localize "STR_House_Sell_Process",true,"fast"] call AYU_Client_fnc_notification_system;};
if (!license_civ_home) exitWith {[ localize "STR_House_License",true,"fast"] call AYU_Client_fnc_notification_system;};
if (count life_houses >= (LIFE_SETTINGS(getNumber,"house_limit"))) exitWith {[ format [localize "STR_House_Max_House",LIFE_SETTINGS(getNumber,"house_limit")],true,"fast"] call AYU_Client_fnc_notification_system;};
closeDialog 0;
...
if (_action) then {
    if (BANK < (_houseCfg select 0)) exitWith {[ format [localize "STR_House_NotEnough"],true,"fast"] call AYU_Client_fnc_notification_system;};
