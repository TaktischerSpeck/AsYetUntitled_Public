...
if (_house getVariable ["garageBought",false]) exitWith {[ localize "STR_Garage_alreadyOwned",true,"fast"] call AYU_Client_fnc_notification_system;};
if ((_house getVariable "house_owner") select 0 != getPlayerUID player) exitWith {[ localize "STR_Garage_NotOwner",true,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_action) then {
    ...
    if (BANK < _price) exitWith {[ format [localize "STR_House_NotEnough"],true,"fast"] call AYU_Client_fnc_notification_system;};
    BANK = BANK - _price;
