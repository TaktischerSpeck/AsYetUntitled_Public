...
if (!([_uid] call life_fnc_isUIDActive)) exitWith {[ localize "STR_House_Raid_OwnerOff",true,"fast"] call AYU_Client_fnc_notification_system;};

_houseInv = _house getVariable ["Trunk",[[],0]];
if (_houseInv isEqualTo [[],0]) exitWith {[ localize "STR_House_Raid_Nothing",true,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_value > 0) then {
    ...
    [ localize "STR_House_Raid_NoIllegal",true,"fast"] call AYU_Client_fnc_notification_system;
};
