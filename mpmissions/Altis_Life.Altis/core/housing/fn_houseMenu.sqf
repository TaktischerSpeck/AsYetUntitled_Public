...
if (_curTarget in life_hideoutBuildings) exitWith {
    closeDialog 0;
    [ localize "STR_House_Hideout",false,"fast"] call AYU_Client_fnc_notification_system;
};
...
if (!(_curTarget in life_vehicles) || isNil {_curTarget getVariable "house_owner"}) then {
    ...
    if (_isHouse) then {
        if (getNumber (missionConfigFile >> "Housing" >> worldName >> (typeOf _curTarget) >> "canGarage") isEqualTo 1) then {
            ...
            _Btn2 buttonSetAction '[ format [localize "STR_pInAct_GarageExtNOTF",[LIFE_SETTINGS(getNumber,"houseGarage_buyPrice")] call life_fnc_numberText],false,"fast"] call AYU_Client_fnc_notification_system;';
            _Btn2 ctrlShow true;
        };
    };
