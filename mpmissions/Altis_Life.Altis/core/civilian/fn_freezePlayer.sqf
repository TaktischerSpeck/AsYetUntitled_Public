...
if (life_frozen) then {
    [ localize "STR_NOTF_Unfrozen",false,"fast"] call AYU_Client_fnc_notification_system;
    [1,format [localize "STR_ANOTF_Unfrozen",profileName]] remoteExecCall ["life_fnc_broadcast",_admin];
    disableUserInput false;
    life_frozen = false;
} else {
    [ localize "STR_NOTF_Frozen",false,"fast"] call AYU_Client_fnc_notification_system;
