...
if (!(_building isKindOf "House_F")) exitWith {[ localize "STR_ISTR_Bolt_NotNear",true,"fast"] call AYU_Client_fnc_notification_system;};
...
if (_door isEqualTo 0) exitWith {[ localize "STR_Cop_NotaDoor",true,"fast"] call AYU_Client_fnc_notification_system;}; //Not near a door to be broken into.
_doorN = _building getVariable [format ["bis_disabled_Door_%1",_door],0];
if (_doorN isEqualTo 1) exitWith {[ localize "STR_House_FedDoor_Locked0",true,"fast"] call AYU_Client_fnc_notification_system;};
