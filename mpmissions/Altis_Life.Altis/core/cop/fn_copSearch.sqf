...
if (!alive _civ || player distance _civ > 5) exitWith {[ format [localize "STR_Cop_CouldntSearch",_civ getVariable ["realname",name _civ]],false,"fast"] call AYU_Client_fnc_notification_system;};
//[ format ["%1",_this],false,"fast"] call AYU_Client_fnc_notification_system;
...
,(_civ getVariable ["realname",name _civ]),_inv,if (_robber) then {"Robbed the bank"} else {""}],false,"fast"] call AYU_Client_fnc_notification_system;
