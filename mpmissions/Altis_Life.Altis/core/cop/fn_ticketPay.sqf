...
if (CASH < life_ticket_val) exitWith {
    if (BANK < life_ticket_val) exitWith {
        [ localize "STR_Cop_Ticket_NotEnough",true,"fast"] call AYU_Client_fnc_notification_system;;
        [1,"STR_Cop_Ticket_NotEnoughNOTF",true,[profileName]] remoteExecCall ["life_fnc_broadcast",life_ticket_cop];
        closeDialog 0;
    };

    [ format [localize "STR_Cop_Ticket_Paid",[life_ticket_val] call life_fnc_numberText],true,"fast"] call AYU_Client_fnc_notification_system;
